/*
 * @Author: Rainy
 * @Date: 2020-06-08 15:26:33
 * @LastEditors: Rainy
 * @LastEditTime: 2020-06-08 19:27:59
 */

// https://github.com/Nijikokun/generate-schema/blob/master/src/schemas/json.js
...
function getPropertyFormat(value: any): string | null {
...
function processArray(array: any[], output?: any, nested?: any) {
  ...
  if (output.items.oneOf) {
    var oneOfList: any[] = [];
    output.items.oneOf.forEach((item: any) => {
      var contains = oneOfList.find(function (one) {
        return isEqual(one, item);
      });
      if (!contains) {
        oneOfList.push(item);
      }
...
function processObject(object: any, output?: any, nested?: any) {
  ...
  for (const key in object) {
    ...
    let format = getPropertyFormat(value);
...
module.exports = function Process(title: any, object: any) {
  let processOutput;
  const output: any = {
    $schema: DRAFT,
  };
